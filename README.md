...
| 字段槽 | 必需 | 说明 |
| --- | --- | --- |
| 门店 | 是 | 用于排序和排名的实体字段，也可以是客户、商品等其他分类字段 |
| 销售额 | 是 | 用于排序和累计的正数指标；视觉对象使用该字段槽中的第一个度量 |
| 工具提示 | 否 | 可添加多个辅助字段或度量，并沿用模型格式 |

示例：

- 门店：`DimPharmacy[pharmacy_id]`
- 销售额：`Measure[销售额]`
- 工具提示：`DimPharmacy[pharmacy_name]`、地区、订单数等

悬停曲线时会显示实体、指标值、X 排名、当前值占比、累计占比，以及所有放入“工具提示”的辅助字段和度量。当前值占比是该实体销售额除以全部有效销售额；累计占比是从第 1 名累计到当前排名的销售额占比。
